...
#include <linux/ktime.h> // ByteExpress Extension
...
// Embeds 64B payload chunks into a given SQ
static inline void nvme_sq_byteexpress_transfer(struct nvme_queue *nvmeq, u8 *payload) {
	void *sq_slot = nvmeq->sq_cmds + (nvmeq->sq_tail << nvmeq->sqes);
	memcpy(sq_slot, payload, sizeof(struct nvme_command));
			 ...
			 const struct blk_mq_queue_data *bd)
{
	...
	if ((dev->ctrl.quirks & NVME_QUIRK_DISABLE_WRITE_ZEROES) && req_op(req) == REQ_OP_WRITE_ZEROES)
		return BLK_STS_SKIP;
  ...
  start = ktime_get_ns();
	...
	if (cmd->common.opcode == 0xA8) { // ByteExpress Command
		int slots_needed = 1 + DIV_ROUND_UP(data_len, 64); // 1 for cmd + N payloads
		nvme_sq_copy_cmd(nvmeq, &iod->cmd);

		void *kaddr = bio_data(req->bio); // PRP-mapped buffer address
		if (!kaddr) {
			printk(KERN_ERR "Failed to get host buffer address from PRP\n");
			spin_unlock(&nvmeq->sq_lock);
			return BLK_STS_IOERR;
		}
		...
		while (offset < data_len) {
			size_t chunk_size = min((size_t)64, data_len - offset);
			u8 temp_payload[64] = {0};
		
			memcpy(temp_payload, kaddr + offset, chunk_size);
			nvme_sq_byteexpress_transfer(nvmeq, temp_payload);
...
static const struct pci_device_id nvme_id_table[] = {
	...
	{ PCI_DEVICE(0x1d97, 0x2263),   /* SPCC */
		.driver_data = NVME_QUIRK_DISABLE_WRITE_ZEROES, },
	{ PCI_DEVICE(0x10ee, 0x7028), /* Cosmos+ OpenSSD */
		.driver_data = NVME_QUIRK_DISABLE_WRITE_ZEROES, },
