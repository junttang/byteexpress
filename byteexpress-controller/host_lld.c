...
///////////////////////////////////////////////// ByteExpress Extension
//#define BYTE_EXPRESS_DEBUG
unsigned int get_nvme_cmd(unsigned short *qID, unsigned short *cmdSlotTag, unsigned int *cmdSeqNum, unsigned int *cmdDword, void *bufAddr) {
    NVME_CMD_FIFO_REG nvmeReg;
    unsigned int *payload_ptr = (unsigned int *)bufAddr; 
    nvmeReg.dword = IO_READ32(NVME_CMD_FIFO_REG_ADDR);

    if(nvmeReg.cmdValid == 1) {
#ifdef BYTE_EXPRESS_DEBUG
        XTime st, ed;
        XTime_GetTime(&st);
#endif
        unsigned int addr;
        ...
        *cmdSeqNum = nvmeReg.cmdSeqNum;
        ...
        for(idx = 0; idx < 16; idx++) {
            unsigned int dword = IO_READ32(addr + (idx * 4));
            *(cmdDword + idx) = dword;
#ifdef BYTE_EXPRESS_DEBUG
            xil_printf("  Transferred DWORD[%2u]: 0x%08X\r\n", idx, dword);
#endif
        }
        ...
        // If ByteExpress-applied cmd, also fetch and dump the next 64B command
        if (opc == IO_NVM_BYTEEXPRESS) {
#ifdef BYTE_EXPRESS_DEBUG
            xil_printf("ByteExpress command detected. Dumping trailing payloads...\r\n");
#endif
            unsigned int i, num_cmds, data_len = cmdDword[13]; 
	    num_cmds = (data_len % 64 == 0) ? (data_len / 64) : (data_len / 64 + 1);  
            for (i = 0; i < num_cmds; i++) {
#ifdef BYTE_EXPRESS_DEBUG
                xil_printf("Trailing payload chunk #%2u\r\n", i);
#endif
                nvmeReg.dword = IO_READ32(NVME_CMD_FIFO_REG_ADDR);
                next_addr = NVME_CMD_SRAM_ADDR + ((nvmeReg.cmdSlotTag) * 64);
                for (idx = 0; idx < 16; idx++) {
                    unsigned int dword = IO_READ32(next_addr + (idx * 4));
                    payload_ptr[(i * 16) + idx] = dword;
#ifdef BYTE_EXPRESS_DEBUG
                    xil_printf("  Inline transferred DWORD[%2u]: 0x%08X\r\n", idx, dword);
#endif
                }
                set_nvme_slot_release(nvmeReg.cmdSlotTag);
            }
        }
#ifdef BYTE_EXPRESS_DEBUG
        XTime_GetTime(&ed);
        u64 cycles = ed - st;
        double exec_ns = ((double)cycles) * 1.0 * 1000000000 / (double)COUNTS_PER_SECOND;
        printf("[ByteExpress] Execution time: %.0f ns\r\n", exec_ns);
#endif
    }
